...
bool IsFood(const SnakeFood & aFood, int aX, int aY)
{
  ...
  return find(aFood.begin(), aFood.end(), potentialFood) != aFood.end();
  ...
  while (true)
  {
    ...
    // check if we have got any new points
    {
      // #STL Implement this block logic this using STL
      {
		...
		*/
		auto it = find_if(snakeFood.begin(), snakeFood.end(), [&snake](const SnakeBlock& x) {return snake.IsHeadOfSnake(x.GetX(), x.GetY()); });
		if (it != snakeFood.end())
		{
			gamePoints++;
			snakeFood.erase(it);
